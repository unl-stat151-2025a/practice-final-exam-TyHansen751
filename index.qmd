...
names(sort(missing_counts)[length(missing_counts) %/% 2])  # Median NAs
...
# Remove NAs
clean_textures <- yarn$texture_clean[!is.na(yarn$texture_clean)]
# Combine all text into one long string
all_text <- paste(clean_textures, collapse = " ")
# Split into individual words
all_words <- strsplit(tolower(all_text), " ")[[1]]
# Count word frequencies
word_counts <- table(all_words)
# Get top 15 words
top_descriptors <- names(sort(word_counts, decreasing = TRUE)[1:15])
top_descriptors
...
is_top_descriptor <- function(text) {
  any(str_detect(text, paste(top_descriptors, collapse = "|")))
}
...
yarn <- yarn %>%
  mutate(yarn_weight_wpi_num = sapply(yarn_weight_wpi, convert_weight_int))
# What is the mean yarn weight for Bernat yarns?
# If you had trouble with 6.4, you can use `prob6pt4.csv` to complete this problem. 
yarn %>%
  filter(yarn_company_name == "Bernat") %>%
  summarize(mean_weight = mean(yarn_weight_wpi_num, na.rm = TRUE))
# Remove any rows with an NA for `yarn_weight_wpi_num`.     
# If you had trouble with 6.4, you can use `prob6pt4.csv` to complete this problem. 
yarn_clean <- yarn %>% filter(!is.na(yarn_weight_wpi_num))
